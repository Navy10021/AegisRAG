...
class AdvancedRAGAnalyzer:
    ...
    def __init__(
        ...
        self._search_cached = lru_cache(maxsize=self.config.CACHE_SIZE)(
            self._search_impl
        )
    ...
    def _initialize_search_components(self, policies: List[SecurityPolicy]):
        ...
        try:
            ...
            if self.embedding_model:
                policy_texts = [
                    f"{p.title}. {p.content}. {' '.join(p.keywords)}" for p in policies
                ]
                self.policy_embeddings = encode_texts(
                    policy_texts, self.embedding_model
                )
    ...
    def analyze(
        ...
        should_use_self_rag = (
            use_self_rag if use_self_rag is not None else self.enable_self_rag
        )
    ...
    def _analyze_with_self_rag(
        ...
        try:
            ...
            # Step 4: 지원도 평가
            support_level = self.self_rag_engine.assess_support(
            ...
            result.confidence_score = min(
                result.confidence_score + confidence_boost, 1.0
            )
    ...
    def _create_breakdown(self, result, similarities):
        ...
        patterns = (
            self.language_detector.get_patterns(result.detected_language)
            if self.language_detector
            else {}
        )
    ...
    def _analyze_llm(self, text: str, policies: List[SecurityPolicy]):
        ...
        sanitized_text = sanitize_prompt_input(
            text, max_length=self.config.MAX_INPUT_LENGTH
        )
...
{{
    ...
    def _analyze_rules(
        ...
        patterns = (
            self.language_detector.get_patterns(language)
            if self.language_detector
            else {}
        )
        ...
        level = (
            "CRITICAL"
            if score >= self.config.RISK_CRITICAL_THRESHOLD
            else (
                "HIGH"
                if score >= self.config.RISK_HIGH_THRESHOLD
                else "MEDIUM" if score >= self.config.RISK_MEDIUM_THRESHOLD else "LOW"
            )
        )
    ...
    def analyze_batch(
        ...
        return [
            self.analyze(t, user_id=uid, use_self_rag=use_self_rag)
    ...
    def _print_self_rag_result(
        ...
        print(
            f"   Utility Score: {self_rag_result.utility_score.value}/5 {'★' * self_rag_result.utility_score.value}"
        )
    ...
    def print_stats(self):
        ...
        if self.relationship_analyzer:
            ...
            print(
                f"🔗 Events: {g.number_of_nodes()} nodes, {g.number_of_edges()} edges"
            )
